...
# from auto_augment_np import AutoAugmentNP
...
class ResNet_valid(nn.Module):

    def __init__(self, block = BasicBlock, configs = [3,4,6,3,64,128,256,512,1], in_planes = 3, num_classes=1000, MEAN = [0.5, 0.5, 0.5], STD = [0.25,0.25,0.25], zero_init_residual=False,
        ...
        layers = configs[:4]
        channels = configs[4:8]
        self.resolution = configs[8]
        # self.aug = configs[9]
        ...
        #                            bias=False)
        # else:
        self.conv1 = nn.Conv2d(in_planes, self.inplanes, kernel_size=7, stride=1, padding=3,
                               bias=False)
    ...
    def forward(self, x):
        # resolution = int(x.shape[2] * self.resolution)
        # resolution = 36
        ...
        #     #     x1[i,:,:,:] = self.augment(x1[i,:,:,:])
        #     # x = torch.from_numpy(x1).permute(0,3,1,2).cuda()
        #     x = torch.from_numpy(x1).cuda()
        # # else:
        # #     x = x1.view(x.shape)
